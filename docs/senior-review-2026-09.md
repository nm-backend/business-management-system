...
### P1 — В полной карточке клиента не было прибыли (требование ТЗ) — ✅ исправлено 2026-09-06

Было: владелец в карточке клиента видел сумму заказов, оплаты и долг, но не прибыль по клиенту — в `apps/clients` не было ни поля, ни расчёта (`profit`/`revenue`/`margin` отсутствовали), хотя ТЗ требует её в полной карточке.

Стало: `ClientViewSet.get_queryset` для owner аннотирует `profit` двумя коррелированными Subquery только по ВЫДАННЫМ заказам (`status=DELIVERED`, не архивным): Σ `total_amount` − Σ `quantity × cost_price` (снимок себестоимости на момент выдачи — ровно как COGS в отчётах). Один SQL-запрос на весь список, без N+1. Поле живёт только в `ClientOwnerSerializer`: админу/менеджеру оно не аннотируется и не сериализуется (server-side, не UI-скрытие). UI: строка «Прибыль» в модалке клиента (зелёная при ≥0, красная при убытке). Тесты: `apps/clients/tests_client_profit_k.py` (7 новых) + `profit` добавлен в запрещённый набор `ClientLeakTests` (`tests_financial_isolation_v2.py`); 55 тестов clients+isolation, i18n parity и 74 vitest — зелёные.

Известное ограничение: возвраты клиентам учитываются статьёй расходов без привязки к клиенту/заказу и не вычитаются из этой прибыли — как и в gross profit отчётов.
