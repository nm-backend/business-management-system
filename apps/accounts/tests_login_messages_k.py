...
class LoginMessageTests(TestCase):
    ...
    def _assert_translated(self, resp, key, lang='uz_cyrl'):
        expected = translate(key, lang)
        self.assertIn(expected, str(resp.data), f'{key} not found in {resp.data}')

    def test_wrong_password_is_generic(self):
        resp = self._login('msg_worker', 'WrongPass123!')
        self.assertEqual(resp.status_code, 400)
        self._assert_translated(resp, 'errors.auth.invalid_credentials')
    ...
    def test_deactivated_account_message(self):
        ...
        self.assertEqual(resp.status_code, 400)
        self._assert_translated(resp, 'errors.auth.account_inactive')
    ...
    def test_blocked_company_message(self):
        ...
        resp = self._login('msg_worker', 'WorkerPass123!')
        self.assertEqual(resp.status_code, 400)
        self._assert_translated(resp, 'errors.auth.company_inactive')
