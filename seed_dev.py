...
prod1, _ = FinishedProduct.objects.get_or_create(name='Стул деревянный', company=company, defaults={'quantity': 10, 'unit': 'sht', 'cost_price': Decimal('12000'), 'sale_price': Decimal('25000'), 'min_stock': 2})
print('Products OK')
...
Order.objects.create(client=client1, product=prod1, quantity=2, unit='izdelie', total_amount=Decimal('50000'), status='new', deadline=timezone.now()+timezone.timedelta(days=7), company=company, paid_amount=Decimal('0'))
Order.objects.create(client=client2, product=prod1, quantity=1, unit='izdelie', total_amount=Decimal('25000'), paid_amount=Decimal('10000'), status='in_progress', deadline=timezone.now()+timezone.timedelta(days=14), company=company)
print('Orders OK')
print('DONE')
